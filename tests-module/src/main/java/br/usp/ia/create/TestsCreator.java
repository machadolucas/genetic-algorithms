...
public class TestsCreator {
    ...
    public void execute() {
        ...
        final Template.CrossoverStrategy[] crossoverStrategies = { Template.CrossoverStrategy.one_point,
                Template.CrossoverStrategy.two_point, Template.CrossoverStrategy.uniform };
        // Probabilidades de crossover
        final String[] crossoverEnergies = { "0.8", "0.4" };

        // Funcoes a serem otimizadas
        final Template.FitnessStrategy[] fitnessStrategies = { Template.FitnessStrategy.bump,
                Template.FitnessStrategy.gold, Template.FitnessStrategy.rastrigin };

        // Mutacoes a serem usadas
        final Template.MutationStrategy[] mutationStrategies = { Template.MutationStrategy.simple,
                Template.MutationStrategy.uniform };
        ...
        // Criterio de parada
        final Template.StopStrategy[] stopStrategies = { Template.StopStrategy.convergence,
                Template.StopStrategy.number_of_generations };

        // Criterio de troca de populacao
        final Template.PopulationChangeStrategy[] populationChangeStrategies = {
                Template.PopulationChangeStrategy.complete_replacement,
                Template.PopulationChangeStrategy.mu_plus_lambda };
                            ...
                            for (final Template.MutationStrategy mutation : mutationStrategies) {
                                ...
                                for (final String mutEnergy : mutationEnergies) {
                                    ...
                                    for (final int popSize : populationSizes) {
                                        ...
                                        for (final Template.PopulationChangeStrategy popChange : populationChangeStrategies) {

                                            // Para cada tipo de criterio de parada
                                            for (final Template.StopStrategy stop : stopStrategies) {

                                                if (stop == Template.StopStrategy.number_of_generations) {
                                                    ...
                                                    for (final int maxGen : maxNumbersOfGenerations) {
                                                        ...
                                                        final String fileContent = Template.getTestFileContent(
                                                                selection, elitism, crossover, crossEnergy, function,
                                                                mutation, mutEnergy, popSize,
                                                                generationsToLogOnScreenInterval, maxGen, stop,
                                                                popChange);
                                                    ...
                                                    final String fileContent = Template.getTestFileContent(
                                                            selection, elitism, crossover, crossEnergy, function,
                                                            mutation, mutEnergy, popSize,
                                                            generationsToLogOnScreenInterval, 0, stop,
                                                            popChange);
